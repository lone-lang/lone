...
static void lone_auxiliary_value_to_table(struct lone_lisp *lone, struct lone_value table, struct lone_value unknowns, struct lone_auxiliary_vector *auxiliary)
{
	...
	case AT_BASE_PLATFORM:
		...
		value = lone_text_from_c_string(lone, auxiliary->value.as.c_string);
		break;
#endif
	...
	case AT_HWCAP:
		...
		value = lone_integer_create(auxiliary->value.as.unsigned_integer);
		break;
#endif
	...
	case AT_BASE:
		...
		value = lone_pointer_create(auxiliary->value.as.pointer, LONE_TO_UNKNOWN);
		break;
#endif
	...
	case AT_EXECFD:
		...
		value = lone_integer_create(auxiliary->value.as.signed_integer);
		break;
#endif
	...
	case AT_RANDOM:
		...
		value = lone_bytes_transfer(lone, auxiliary->value.as.bytes, 16, false);
		break;
#endif
	...
	default:
		key = lone_integer_create(auxiliary->type);
		value = lone_integer_create(auxiliary->value.as.unsigned_integer);
...
{
	struct lone_value table = lone_table_create(lone, 32, lone_nil());
	struct lone_value unknowns = lone_table_create(lone, 2, lone_nil());
	size_t i;
...
{
	struct lone_value table = lone_table_create(lone, 64, lone_nil()), key, value;
	...
	for (/* c_strings */; *c_strings; ++c_strings) {
		...
		key = lone_text_from_c_string(lone, c_string_key);
		value = lone_text_from_c_string(lone, c_string_value);
		lone_table_set(lone, table, key, value);
	}
...
static struct lone_value lone_arguments_to_vector(struct lone_lisp *lone, int count, char **c_strings)
{
	...
	arguments = lone_vector_create(lone, (size_t) count);

	for (i = 0; i < count; ++i) {
		lone_vector_set(lone,
		                arguments,
		                lone_integer_create(i),
		                lone_text_from_c_string(lone, c_strings[i]));
	}
...
static void lone_fill_linux_system_call_table(struct lone_lisp *lone, struct lone_value linux_system_call_table)
{
	...
	for (i = 0; i < (sizeof(linux_system_calls)/sizeof(linux_system_calls[0])); ++i) {
		lone_table_set(lone, linux_system_call_table,
		               ...
		               lone_integer_create(linux_system_calls[i].number));
	}
}
...
{
	struct lone_value name, module, linux_system_call_table, count, arguments, environment, auxiliary_vector, primitive;
	struct lone_function_flags flags;

	name = lone_intern_c_string(lone, "linux");
	module = lone_module_for_name(lone, name);
	linux_system_call_table = lone_table_create(lone, 1024, lone_nil());
	...
	count = lone_integer_create(argc);
...
{
	struct lone_value linux_system_call_table, argument;
	...
	return lone_integer_create(result);
}
